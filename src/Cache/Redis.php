...
namespace Kindness\ModuleManage\Cache;
...
class Redis
{
    ...
    public static function instance()
    {
        if (!static::$instance) {
            ...
            static::$instance = new RedisManager(app(), $client, $config);
        }
     ...
     * Author:puweizhi
     */
    public static function setArray($key, $value, $ttl = 3600): void
    {
    ...
    public static function getArray($key)
    {
        ...
        $value_seri = unserialize($value);
        if (is_object($value_seri) || is_array($value_seri)) {
            return $value_seri;
        }
        return $value;
    }

    public static function handleDuplicateRequest(string $uniqueKey, int $ttl = 5): bool
    {
        $key = 'lock-key-' . $uniqueKey;
        if (Redis::get($key)) {
            return false;
        }
        //缓存时间
